...
class DataHandler(object):
    ...
    def load_service_data_and_calulating(self, host_obj, trigger, REDIS_CONN):
        ...
        for expression in self.redis.triggerexpression_set.select_related().order_by('id'):
            ...
            if single_expression_res:
                ...
                if single_expression_res['calc_res']:
        ...
        print("whole trigger res:", self.rides.name, expression_res_string)
        if expression_res_string:
            ...
            if trigger_res:  # 终于走到这一步,该触发报警了
                print("##############trigger alert:", self.rides.severity, trigger_res)
                self.trigger_notifier(host_obj, self.rides.id, positive_expressions,
                                      msg=self.rides.name)  # msg 需要专门分析后生成, 这里是临时写的
